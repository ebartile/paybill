...
import { DocsButton } from 'components/ui'
...
import { BASE_PATH, buildPathWithParams, getAccessToken, getReturnToPath, paybillClient } from '~/common'
  ...
  useEffect(() => {
    paybillClient
      ...
      .then(async ({ error }: { error?: unknown }) => {
        ...
        if (token) {
          const { data, error } = await paybillClient.mfa.getAuthenticatorAssuranceLevel()
          if (error) {
            // if there was a problem signing in via the url, don't redirect
            return
          }
          ...
          <nav className="relative flex items-center justify-between sm:h-10">
            <div className="flex items-center flex-grow flex-shrink-0 lg:flex-grow-0">
              <div className="flex items-center justify-between w-full md:w-auto">
                <Link href={logoLinkToMarketingSite ? 'https://paybill.dev' : '/organizations'}>
                  <Image
                    src={
                      resolvedTheme?.includes('dark')
                        ? `${BASE_PATH}/logo-wordmark--dark.svg`
                        : `${BASE_PATH}/logo-wordmark--light.svg`
                    }
                    alt="Paybill Logo"
                    height={24}
                    width={120}
            ...
            <div className="items-center hidden space-x-3 md:ml-10 md:flex md:pr-4">
              <DocsButton abbrev={false} href="https://docs.paybill.dev/docs" />
            </div>
          </nav>
        </div>
            ...
            {showDisclaimer && (
              <div className="sm:text-center">
                <p className="text-xs text-foreground-lighter sm:mx-auto sm:max-w-sm">
                  By continuing, you agree to Paybill's{' '}
                  <Link
                    href="https://paybill.dev/legal/terms"
                  ...
                  and{' '}
                  <Link
                    href="https://paybill.dev/legal/privacy"
                    className="underline hover:text-foreground-light"
                  >
                    Privacy Policy
                  </Link>
