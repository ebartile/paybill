...
import { AlertError, Button, ButtonTooltip, ShimmeringLoader, Table } from '~/components/ui'
import PartnerManagedResource from '~/components/layouts/PartnerManagedResource'
...
const InvoicesSettings = () => {
  ...
  const { data: count, isError: isErrorCount } = useInvoicesCountQuery(
    ...
    { enabled: selectedOrganization?.managed_by === 'paybill' }
  )
  ...
  if (
    ...
    selectedOrganization?.managed_by !== 'paybill'
  ) {
    return (
