...
import { Admonition, AlertError, Button, Loading, Table, Tooltip, TooltipContent, TooltipTrigger } from '~/components/ui'
import { MemberRow } from './MemberRow'
import { useParams } from '~/hooks/useParams'
import { useProfile } from '~/components/layouts/Profile'
...
const MembersView = ({ searchString }: MembersViewProps) => {
  ...
  const [[user], otherMembers] = partition(
    filteredMembers,
    (m) => m.paybill_id === profile?.paybill_id
  )
  const sortedMembers = otherMembers.sort((a, b) =>
    (a.primary_email ?? '').localeCompare(b.primary_email ?? '')
  )

  const userMember = members.find((m) => m.paybill_id === profile?.paybill_id)
            ...
            <Table
              ...
              body={[
                ...(isSuccessRoles && isSuccessMembers && !isOrgScopedRole
                  ...
                  : []),
                ...(!!user ? [<MemberRow key={user.paybill_id} member={user} />] : []),
                ...sortedMembers.map((member) => (
                  <MemberRow key={member.paybill_id} member={member} />
                )),
