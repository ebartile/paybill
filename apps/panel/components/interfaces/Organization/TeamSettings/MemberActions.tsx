...
export const MemberActions = ({ member }: MemberActionsProps) => {
  ...
  const memberIsUser = member.paybill_id == profile?.paybill_id
  const orgScopedRoles = allRoles?.org_scoped_roles ?? []
  const projectScopedRoles = allRoles?.project_scoped_roles ?? []
  const isPendingInviteAcceptance = !!member.invited_id

  const userMemberData = members?.find((m) => m.paybill_id === profile?.paybill_id)
  const hasOrgRole =
  ...
  const handleMemberDelete = () => {
    ...
    if (!member.paybill_id) return console.error('paybill_id is required')
    deleteOrganizationMember({ slug, paybillId: member.paybill_id })
  }
