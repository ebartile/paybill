...
import type { AuthError } from '@paybilldev/auth-js'
...
import { buildPathWithParams, getReturnToPath, paybillClient } from '~/common'
...
const SignInForm = () => {
  ...
  const onSignIn = async ({ email, password }: { email: string; password: string }) => {
    ...
    const { error } = await paybillClient.signInWithPassword({
      email,
      password,
