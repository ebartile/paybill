...
import { Button, Form_Shadcn_, FormControl_Shadcn_, FormField_Shadcn_, Input_Shadcn_ } from '~/components/ui'
import { BASE_PATH, paybillClient } from '~/common'
import { FormItemLayout } from '~/components/layouts/Form/FormItemLayout'
...
const ConfirmResetCodeForm = ({ email }: { email: string }) => {
  ...
  const onCodeEntered: SubmitHandler<CodeFormData> = async (data) => {
    ...
    const {
      data: { user },
      error,
    } = await paybillClient.verifyOtp({ email, token: data.code, type: 'recovery' })
