import { createClient } from '@paybilldev/paybill-js'
...
export const uploadAttachment = async (
  ...
  const paybillClient = createClient(SUPPORT_API_URL, SUPPORT_API_KEY, {
    auth: {
  ...
  const { data: file, error } = await paybillClient.storage
    .from(bucket)
  ...
  if (file && getUrl) {
    const { data } = await paybillClient.storage.from(bucket).getPublicUrl(file.path)
