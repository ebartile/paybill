...
import type { AuthMFAVerifyResponse, MFAChallengeAndVerifyParams } from '@paybilldev/auth-js'
...
export const mfaChallengeAndVerify = async (params: MFAChallengeAndVerifyParams) => {
  const { error, data } = await paybillClient.mfa.challengeAndVerify(params)
