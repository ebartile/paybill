...
build_table_rows() {
    ...
    for tag in "${tags[@]}"; do
        ...
        local row="| [${tag}](https://hub.docker.com/layers/paybilldev/paybill/${tag}/images/sha256-${sha}?context=explore) | ${date} | \`docker pull paybilldev/paybill:${tag}\` |"
        rows+=("$row")
    done
    ...
    local latest_row="| Latest EE-LTS | N/A | \`docker pull paybilldev/paybill:ee-lts-latest\` |"
...
main() {
    log "🚀 Starting Paybill LTS table regeneration..."
