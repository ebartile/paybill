...
describe("controller", () => {
	it("action", async () => {
		...
		expect(context.controller).toBeInstanceOf(Controller);
	...
	it("require module", async () => {
		...
		controller.define({
			name: "test",
			middleware: (await import("./middlewares/demo0.js")).default,
			actions: {
				list: (await import("./actions/demo0.js")).default,
			},
		});
	...
	it("require module", async () => {
		...
		controller.define({
			name: "test",
			middleware: (await import("./middlewares/demo1.js")).default,
			actions: {
				list: (await import("./actions/demo1.js")).default,
			},
		});
		const context = {
			arr: [],
	...
	it("require module", async () => {
		...
		controller.define({
			...
			actions: {
				list: {
					handler: (await import("./actions/demo1.js"))
						.default as unknown as HandlerType,
				},
			},
		});
		const context = {
	...
	it("add action", async () => {
		...
		controller.getResource("test").addAction("new", async function (ctx, next) {
			ctx.arr.push(100);
			await next();
			ctx.arr.push(101);
		});
