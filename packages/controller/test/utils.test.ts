...
describe("utils", () => {
	describe("parseQuery", () => {
		it("filter support normal json type", () => {
			...
			expect(
				parseQuery(`filter=${encodeURIComponent(json)}&sort=-col`),
			).toEqual({
				filter: object,
				sort: "-col",
			});
	...
	describe("mergeFields", () => {
		describe("empty default", () => {
			it('always contains "appends"', async () => {
				expect(mergeFields({}, { only: ["col"] })).toMatchObject({
					appends: [],
					only: ["col"],
				});
			});
			...
			it("appends", async () => {
				expect(
					mergeFields({}, { only: ["col1"], appends: ["col2"] }),
				).toMatchObject({
					only: ["col1"],
					appends: ["col2"],
				});
	...
	describe("parseRequest", () => {
		...
		it("decode path", () => {
			const params = parseRequest({
				path: "/posts/foo%20bar/comments",
				method: "POST",
			});
			expect(params).toMatchObject({
				resourceName: "comments",
				associatedName: "posts",
				associatedIndex: "foo bar",
				actionName: "create",
			});
		});
		...
		it("actions options", () => {
			const params = parseRequest({
				path: "/controller/user.posts:list",
				method: "GET",
			});
			expect(params).toMatchObject({
				associatedName: "user",
