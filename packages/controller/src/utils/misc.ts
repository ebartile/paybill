...
export function parseRequest(
	...
	for (const path in defaults[type]) {
		...
		params.actionName = _.get(defaults, [
			type,
			path,
...
export function mergeFields(defaults: any, inputs: any) {
	...
	if (inputs.only) {
		...
		if (defaults.only) {
			fields.only = defaults.only.filter((field: string) =>
				inputs.only.includes(field),
			);
		} else if (defaults.except) {
			fields.only = inputs.only.filter(
				(field: string) => !defaults.except.includes(field),
			);
		} else {
		...
		if (defaults.only) {
			fields.only = defaults.only.filter(
				(field: string) => !inputs.except.includes(field),
			);
		} else {
			fields.except = _.uniq([
				...(inputs.except || []),
				...(defaults.except || []),
			]);
		}
