...
export class Controller {
	...
	constructor(
		...
		private fileLoader: FileLoader = new NodeFileLoader(),
	) {
	...
	middleware({
		...
		return async function controllerMiddleware(
			...
			try {
				...
				ctx.action = self
					.getAction(getNameByParams(params), params.actionName)
					.clone();
				...
				if (
					pathToRegexp("/controller/:rest(.*)").test(ctx.request?.path || "")
				) {
