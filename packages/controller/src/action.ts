 ...
 */
export type FieldsOptionsFn = (
	ctx: ActionContext,
) => FieldsOptions | Promise<FieldsOptions>;
...
export type FilterOptionsFn = (
	ctx: ActionContext,
) => FilterOptions | Promise<FieldsOptions>;
...
export type ParamsCallback = (
	ctx: ActionContext,
) => ActionParams | Promise<ActionParams>;
	...
	wrapMiddlewareWithLogging(fn, logger?) {
		...
		return async (ctx, next) => {
			...
			await fn(ctx, async () => {
				...
				logger.trace(
					`--> Before next middleware: ${name} - ${beforeNext - start}ms`,
					{ reqId },
				);
				...
				logger.trace(
					`<-- After next middleware: ${name} - ${afterNext - beforeNext}ms`,
					{ reqId },
				);
			});
