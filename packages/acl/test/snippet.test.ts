import { SnippetManager, ACL } from "@paybilldev/acl";
...
describe("acl snippet", () => {
	...
	it("should register snippet", () => {
		...
		adminRole.snippets.add("sc.*");
		expect(
			acl.can({ role: "admin", resource: "collections", action: "list" }),
		).not.toBeNull();
		expect(adminRole.snippetAllowed("collections:list")).toBe(true);

		adminRole.snippets.add("!sc.collection-manager.gi");
		expect(
			acl.can({ role: "admin", resource: "gi", action: "list" }),
		).toBeNull();
		expect(adminRole.snippetAllowed("gi:list")).toBe(false);

		expect(
			acl.can({ role: "admin", resource: "fields", action: "list" }),
		).not.toBeNull();
	...
	it("should return true when last rule allowd", () => {
		...
		expect(
			acl.can({ role: "admin", resource: "users", action: "list" }),
		).toBeNull();
	});
});
...
describe("snippet manager", () => {
	describe("allow method", () => {
		...
		it("should return null when not matched", () => {
			...
			expect(
				snippetManager.allow("fields:list", "sc.collection-manager.fields"),
			).toBeNull();
		});
