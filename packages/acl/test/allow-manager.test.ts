import { ACL } from "@paybilldev/acl";
...
describe("AllowManager tests", () => {
	...
	it("should allow a specific resource and action", async () => {
		...
		expect(
			await acl.allowManager.isAllowed("documents", "read", {}),
		).toBeTruthy();
		expect(
			await acl.allowManager.isAllowed("documents", "write", {}),
		).toBeFalsy();
	...
	it("should allow multiple actions for a single resource", async () => {
		...
		expect(
			await acl.allowManager.isAllowed("documents", "delete", {}),
		).toBeFalsy();
	});
	...
	it("should allow action with a condition function", async () => {
		...
		expect(
			await acl.allowManager.isAllowed("documents", "edit", { user: "admin" }),
		).toBeTruthy();
		expect(
			await acl.allowManager.isAllowed("documents", "edit", { user: "guest" }),
		).toBeFalsy();
	});

	it("should allow action with a registered named condition", async () => {
		acl.allowManager.registerAllowCondition(
			"isAdmin",
			(ctx) => ctx.user === "admin",
		);
		...
		expect(
			await acl.allowManager.isAllowed("settings", "update", { user: "admin" }),
		).toBeTruthy();
		expect(
			await acl.allowManager.isAllowed("settings", "update", { user: "guest" }),
		).toBeFalsy();
	});
	...
	it("should return false for unknown resource or action", async () => {
		...
		expect(
			await acl.allowManager.isAllowed("documents", "unknown", {}),
		).toBeFalsy();
	});
	...
	it("should handle multiple conditions for a single action", async () => {
		...
		expect(
			await acl.allowManager.isAllowed("documents", "delete", { owner: true }),
		).toBeTruthy();
		expect(
			await acl.allowManager.isAllowed("documents", "delete", {}),
		).toBeTruthy();
		expect(
			await acl.allowManager.isAllowed("documents", "delete", { owner: false }),
		).toBeTruthy();
	});
	...
	it("should allowConfigure for a role with allowConfigure strategy", async () => {
		...
		expect(
			await acl.allowManager.isAllowed("settings", "update", {}),
		).toBeTruthy();
	});
