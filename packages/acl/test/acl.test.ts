...
describe("acl", () => {
	...
	beforeEach(() => {
		acl = new ACL({
			currentUser: "d6f8c3a2-1b5e-4f7a-9f3d-2e7b6c4d8a91",
			currentRole: "admin",
		});
	});
	...
	it("should grant action with own params", () => {
		...
		expect(canResult).toMatchObject({
			...
			action: "edit",
			params: {
				filter: { createdById: "d6f8c3a2-1b5e-4f7a-9f3d-2e7b6c4d8a91" },
			},
		});
	});
	...
	it("should allow all", () => {
		...
		expect(
			acl.can({ role: "admin", resource: "posts", action: "create" }),
		).toMatchObject({
			role: "admin",
			resource: "posts",
			action: "create",
	...
	it("should work with alias action", () => {
		...
		expect(
			acl.can({ role: "admin", resource: "posts", action: "get" }),
		).toMatchObject({
			role: "admin",
			resource: "posts",
			action: "get",
		...
		expect(
			acl.can({ role: "admin", resource: "posts", action: "list" }),
		).toMatchObject({
			role: "admin",
			resource: "posts",
			action: "list",
	...
	it("should not allow when strategyResources is set", async () => {
		...
		expect(
			acl.can({ role: "admin", resource: "users", action: "create" }),
		).toBeTruthy();
	...
	it("should run beforeGrantAction when granting an action", () => {
		...
		acl.beforeGrantAction((ctx) => {
			if (ctx.path === "posts:update") {
				ctx.params = {
					filter: { status: "approved", updatedBy: acl.getCurrentUser() },
				};
			}
		});
		...
		expect(result).toMatchObject({
			...
			params: {
				filter: {
					status: "approved",
					updatedBy: "d6f8c3a2-1b5e-4f7a-9f3d-2e7b6c4d8a91",
				},
			},
		});
	});
	...
	it("should also apply beforeGrantAction when grantAction is called after role creation", () => {
		...
		expect(result).toMatchObject({
			...
			action: "publish",
			params: {
				filter: { publishedBy: "d6f8c3a2-1b5e-4f7a-9f3d-2e7b6c4d8a91" },
			},
		});
	});
});
