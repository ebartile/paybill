...
export class AllowManager {
	protected skipActions = new Map<
		string,
		Map<string, string | ConditionFunc | true>
	>();
	...
	allow(
		...
		const actionMap =
			this.skipActions.get(resourceName) ||
			new Map<string, string | ConditionFunc>();
					...
					if (condition) {
						results.push(
							typeof condition === "string"
								? this.registeredCondition.get(condition)
								: condition,
						);
					}
				}
	...
	async isAllowed(resourceName: string, actionName: string, ctx: any) {
		const skippedConditions = this.getAllowedConditions(
			resourceName,
			actionName,
		);
