	...
	afterBuild(db: Database) {
		...
		if (collection && collection.isParent()) {
			...
			OptionsParser.appendInheritInspectAttribute(
				lodash.isArray(this.attributes)
					? this.attributes
...
export class EagerLoadingTree {
	...
	static buildFromSequelizeOptions(options: {
		...
		const { model, rootAttributes, includeOption, db, rootQueryOptions } =
			options;
		...
		const traverseIncludeOption = (includeOption, eagerLoadingTreeParent) => {
			...
			if (includeOption.length > 0) {
				const modelPrimaryKey =
					eagerLoadingTreeParent.model.primaryKeyAttribute;
	...
	async load(transaction?: Transaction) {
		...
		const loadRecursive = async (node, ids = []) => {
			...
			if (!node.parent) {
				// load root instances
				const rootInclude =
					this.rootQueryOptions?.include || node.includeOption;

				const includeForFilter = rootInclude.filter((include) => {
					return (
						Object.keys(include.where || {}).length > 0 ||
						JSON.stringify(this.rootQueryOptions?.filter)?.includes(
							include.association,
						)
					);
				});
				...
				const belongsToAssociationsOnly = isBelongsToAssociationOnly(
					includeForFilter,
					node.model,
				);
				...
				if (belongsToAssociationsOnly) {
					...
					const primaryKeyField =
						node.model.primaryKeyField || node.model.primaryKeyAttribute;

					if (!primaryKeyField) {
						throw new Error(
							`Model ${node.model.name} does not have primary key`,
						);
					}
				...
				for (const [name, _association] of Object.entries(associations)) {
				...
				const otherFindOptions =
					lodash.pick(node.includeOption, ["sort"]) || {};
				...
				if (associationType == "HasOne" || associationType == "HasMany") {
					...
					const foreignKeyValues = node.parent.instances.map((instance) =>
						instance.get(association.sourceKey),
					);
				...
				if (associationType === "BelongsToArray") {
					...
					let where: any = {
						[targetKey]: Array.from(new Set(flatten(targetKeyValues))),
					};
				...
				if (associationType == "BelongsTo") {
					const foreignKey = association.foreignKey;
					const parentInstancesForeignKeyValues = node.parent.instances.map(
						(instance) => instance.get(foreignKey),
					);
					...
					if (node.includeOption.recursively && instances.length > 0) {
						...
						const parentInstances = await node.model.findAll({
							transaction,
							where: {
								[association.targetKey]: results.map(
						...
						const setInstanceParent = (instance) => {
							const parentInstance = parentInstances.find(
								(parentInstance) =>
									parentInstance.get(targetKey) == instance.get(foreignKey),
							);
							if (!parentInstance) {
								return;
							}
							...
							instance[association.as] = instance.dataValues[association.as] =
								parentInstance;
						};
			...
			for (const child of node.children) {
				const modelPrimaryKey =
					node.model.primaryKeyField || node.model.primaryKeyAttribute;
				const nodeIds = instances.map((instance) =>
					instance.get(modelPrimaryKey),
				);
			...
			if (!node.parent) {
				...
				const setParentAccessor = (parentInstance) => {
					...
					if (association.isSingleAssociation) {
						...
						parentInstance[key] = parentInstance.dataValues[key] = isEmpty
							? null
							: children;
					} else {
				...
				if (associationType == "HasMany" || associationType == "HasOne") {
					...
					for (const instance of node.instances) {
						const parentInstance = node.parent.instances.find(
							(parentInstance) =>
								parentInstance.get(sourceKey) == instance.get(foreignKey),
						);
				...
				if (associationType === "BelongsToArray") {
					...
					const instanceMap = node.instances.reduce(
						(mp: { [targetKey: string]: Model }, instance: Model) => {
					...
					node.parent.instances.forEach((parentInstance: Model) => {
						...
						parentInstance.setDataValue(
							association.as,
							targetKeys
								?.map((targetKey: any) => instanceMap[targetKey])
								.filter(Boolean),
						);
					});
				}
				...
				if (associationType == "BelongsTo") {
					...
					for (const instance of node.instances) {
						const parentInstances = node.parent.instances.filter(
							(parentInstance) =>
								parentInstance.get(foreignKey) == instance.get(targetKey),
						);
				...
				if (associationType == "BelongsToMany") {
					...
					for (const instance of node.instances) {
						...
						const parentInstance = node.parent.instances.find(
							(parentInstance) =>
								parentInstance.get(sourceKey) ==
								instance.dataValues[as].get(foreignKey),
						);
		...
		const appendChildCollectionName =
			appendChildCollectionNameAfterRepositoryFind(this.db);
		...
		const setInstanceAttributes = (node) => {
			...
			for (const instance of node.instances) {
				instance.dataValues = lodash.pick(
					instance.dataValues,
					includeAttributes,
				);
			}
