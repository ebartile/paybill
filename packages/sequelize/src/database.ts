...
import { AsyncEmitter, patchSequelizeQueryInterface, snakeCase } from "./utils";
...
export class Database extends AsyncEmitter {
	...
	constructor(options: DatabaseOptions) {
		...
		if (options.logging && options.benchmark == true) {
			opts.benchmark = true;
		}
		...
		this.sequelize.beforeDefine((model, opts) => {
			if (this.options.tablePrefix) {
				if (
					opts.tableName &&
					opts.tableName.startsWith(this.options.tablePrefix)
				) {
					return;
				}
	...
	createMigrator({ migrations }) {
		...
		return new Umzug({
			...
			migrations: Array.isArray(migrations)
				? lodash.sortBy(migrations, (m) => m.name)
				: migrations,
			context,
			storage: new SequelizeStorage({
				tableName: `${this.options.tablePrefix || ""}migrations`,
	...
	getRepository<R extends RelationRepository>(
		name: string,
		relationId: TargetKey,
	): R;

	getRepository<R extends ArrayFieldRepository>(
		name: string,
		relationId: TargetKey,
	): R;
	...
	buildField(options, context: FieldContext) {
		...
		if (
			options.field &&
			collection.options.underscored &&
			!collection.isView()
		) {
	...
	async clean(options: CleanOptions) {
		...
		if (this.options.schema) {
			const tableNames = (
				await this.sequelize.getQueryInterface().showAllTables()
			).map((table) => {
	...
	isSqliteMemory() {
		return (
			...
			lodash.get(this.options, "storage") == ":memory:"
		);
	}

	/* istanbul ignore next -- @preserve */
	async auth(
		options: Omit<QueryOptions, "retry"> & {
		...
		const authenticate = async () => {
			try {
				...
				this.logger.warn(
					`attempt ${attemptNumber}/${retry}: Unable to connect to the database: ${error.message}`,
					{
						method: "auth",
					},
				);
				const nextDelay =
					startingDelay * Math.pow(timeMultiple, attemptNumber - 1);
				attemptNumber++;
				if (attemptNumber < (retry as number)) {
					this.logger.warn(`will retry in ${nextDelay}ms...`, {
						method: "auth",
					});
				}
	...
	async reconnect() {
		...
		const ConnectionManager =
			// @ts-ignore
			this.sequelize.dialect.connectionManager.constructor;
	...
	on(
		event: ModelCreateWithAssociationsEventTypes,
		listener: CreateWithAssociationsListener,
	): this;

	on(
		event: ModelUpdateWithAssociationsEventTypes,
		listener: UpdateWithAssociationsListener,
	): this;

	on(
		event: ModelSaveWithAssociationsEventTypes,
		listener: SaveWithAssociationsListener,
	): this;

	on(
		event: DatabaseBeforeDefineCollectionEventType,
		listener: BeforeDefineCollectionListener,
	): this;

	on(
		event: DatabaseAfterDefineCollectionEventType,
		listener: AfterDefineCollectionListener,
	): this;
			...
			async onDump(dumper, collection: Collection) {
				try {
					const viewDef = await collection.db.queryInterface.viewDef(
						collection.getTableNameWithSchemaAsString(),
					);
