...
import { Model as SequelizeModel, type ModelStatic } from "sequelize";
	...
	implements IModel
{
	...
	public toJSON<T extends TModelAttributes>(): T {
		const handleObj = (obj, options: JSONTransformerOptions) => {
			...
			return handles.reduce(
				(carry, fn) => fn.apply(this, [carry, options]),
				obj,
			);
		};
			...
			for (const key of Object.keys(data)) {
				...
				if (model.hasAlias(key)) {
					...
					if (
						...
						result[key] = handleArray(data[key], opts).map((item) =>
							traverseJSON(item, opts),
						);
	...
	private handleBigInt(obj, options) {
		...
		const bigIntKeys = Object.keys(options.model.rawAttributes).filter(
			(key) => {
				return (
					options.model.rawAttributes[key].type.constructor.name === "BIGINT"
				);
			},
		);
