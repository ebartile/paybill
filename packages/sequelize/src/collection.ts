...
import {
	type ModelOptions,
	type ModelStatic,
	type QueryInterfaceDropTableOptions,
	...
	return function (method: Function, context: ClassMethodDecoratorContext) {
		return function (...args: any[]) {
			...
			try {
				...
				const createdAssociationKeys = lodash.difference(
					afterAssociationKeys,
					beforeAssociationKeys,
				);
...
export interface CollectionOptions
	extends Omit<ModelOptions, "name" | "hooks"> {
	name: string;
	title?: string;
	namespace?: string;
...
> extends EventEmitter {
	...
	constructor(options: CollectionOptions, context: CollectionContext) {
		...
		this.db.tableNameCollectionMap.set(
			this.getTableNameWithSchemaAsString(),
			this,
		);
	...
	get titleField() {
		return (
			(this.options.titleField as string) || this.model.primaryKeyAttribute
		);
	}
	...
	isMultiFilterTargetKey() {
		return (
			Array.isArray(this.filterTargetKey) && this.filterTargetKey.length > 1
		);
	}
	...
	modelInit() {
			...
			get: function () {
				const singleFilterTargetKey: string = (() => {
					...
					if (
						...
						collection.options.filterTargetKey.length === 1
					) {
				...
				if (
					!this._primaryKeyAttribute &&
					singleFilterTargetKey &&
					collection.getField(singleFilterTargetKey)
				) {
		...
		Object.defineProperty(this.model, "primaryKeyAttributes", {
			get: function () {
				if (
					Array.isArray(this._primaryKeyAttributes) &&
					this._primaryKeyAttributes.length
				) {
	...
	checkFieldType(name: string, options: FieldOptions) {
		...
		const isContextTypeMatch = (data, dataType: string): boolean => {
			return [data.dataType?.key, data.type?.toUpperCase()].includes(
				dataType?.toUpperCase(),
			);
		};
		if (
			options.type === "context" &&
			isContextTypeMatch(field, options.dataType)
		) {
			return;
		}
		if (
			field.type === "context" &&
			isContextTypeMatch(options, field.dataType.key)
		) {
			return;
		}

		throw new Error(
			`fields with same column must be of the same type ${JSON.stringify(options)}`,
		);
	}
	...
	setField(name: string, options: FieldOptions): Field {
		...
		if (options.source) {
			...
			if (!sourceCollection) {
				...
				if (!sourceField) {
					...
					options = {
						...lodash.omit(sourceField.options, ["name", "primaryKey"]),
						...options,
					};
				}
			}
		...
		if (
			oldField &&
			oldField.options.inherit &&
			field.typeToString() != oldField.typeToString()
		) {
			throw new Error(
		...
		if (this.isParent()) {
			for (const child of this.context.database.inheritanceMap.getChildren(
				this.name,
				{
					deep: false,
				},
	...
	async removeFieldFromDb(name: string, options?: QueryInterfaceOptions) {
		...
		if (this.model.options.timestamps !== false) {
			// Timestamps related fields are not deleted
			let timestampsFields = ["createdAt", "updatedAt", "deletedAt"];
			if (this.db.options.underscored) {
				timestampsFields = timestampsFields.map((fieldName) =>
					snakeCase(fieldName),
				);
			}
		...
		// Sorting fields are controlled by sortable
	...
	async removeFromDb(
		options?: QueryInterfaceDropTableOptions & { dropCollection?: boolean },
	) {
		if (
			!this.isView() &&
	...
	addIndex(
		...
		this.model._indexes = this.model.options.indexes
			.map((index) =>
				// @ts-ignore
				Utils.nameIndex(this.model._conformIndex(index), tableName),
			)
			.map((item) => {
			...
			indexes
				...
				.map((item) => {
					item.fields = item.fields.map(
						(field) => this.model.rawAttributes[field].field,
					);
					return item;
				}),
			"name",
	...
	public getRealTableName(quoted = false) {
		...
		return !quoted
			? realname
			: this.db.sequelize.getQueryInterface().quoteIdentifiers(realname);
	}
	...
	public getRealFieldName(name: string, quoted = false) {
		...
		return !quoted
			? name
			: this.db.sequelize.getQueryInterface().quoteIdentifier(realname);
	}
	...
	private checkTableName() {
		...
		for (const [_k, collection] of this.db.collections) {
			if (
				...
				throw new Error(
					`collection ${collection.name} and ${this.name} have same tableName "${tableName}"`,
				);
			}
		}
	}
