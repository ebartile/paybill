...
import {
	HasOne,
	type MultiAssociationAccessors,
	Sequelize,
	Transaction,
} from "sequelize";
...
export abstract class MultipleRelationRepository extends RelationRepository {
	...
	async count(options?: CountOptions) {
		...
		const include = queryOptions.include?.filter(
			(item: { association: string }) => {
			...
			attributes: [
				[
					Sequelize.fn(
						...
						Sequelize.fn(
							"DISTINCT",
							Sequelize.col(`${this.targetModel.name}.${this.targetKey()}`),
						),
					),
					"count",
				],
	...
	async remove(
		...
		const transaction = await this.getTransaction(options);
		const sourceModel = await this.getSourceModel(transaction);
		await sourceModel[this.accessors().removeMultiple](
			this.convertTks(options),
			{
				transaction,
			},
		);
	...
	async update(options?: UpdateOptions): Promise<any> {
		...
		for (const instance of instances) {
			if (options.hooks !== false) {
				await this.db.emitAsync(
					`${this.targetCollection.name}.afterUpdateWithAssociations`,
					instance,
					{
						...options,
						transaction,
				...
				await this.db.emitAsync(
					`${this.targetCollection.name}.afterSaveWithAssociations`,
					instance,
					{
						...options,
						transaction,
	...
	protected async destroyByFilter(
		...
		return await this.destroy({
			filterByTk: instances.map((instance) =>
				instance.get(this.targetCollection.filterTargetKey),
			),
			transaction,
		});
	}
