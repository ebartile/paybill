...
export abstract class RelationRepository {
	...
	constructor(
		...
		this.targetCollection =
			this.sourceCollection.context.database.modelCollection.get(
				this.targetModel,
			);
	}
	...
	setSourceKeyValue(sourceKeyValue: TargetKey) {
		this.sourceKeyValue =
			typeof sourceKeyValue === "string"
				? this.decodeMultiTargetKey(sourceKeyValue) || sourceKeyValue
				: sourceKeyValue;
	}
		...
		if (instance) {
			return await this.update({
				filterByTk: instance.get(
					this.targetCollection.filterTargetKey ||
						this.targetCollection.model.primaryKeyAttribute,
				),
				values,
				transaction,
				hooks,
	...
	async create(options?: CreateOptions): Promise<any> {
		...
		if (options.hooks !== false) {
			await this.db.emitAsync(
				`${this.targetCollection.name}.afterCreateWithAssociations`,
				instance,
				{
					...options,
					transaction,
