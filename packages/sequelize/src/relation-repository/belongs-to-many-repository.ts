...
export class BelongsToManyRepository extends MultipleRelationRepository {
	async aggregate(options: AggregateOptions) {
		...
		const sourceModel = await this.getSourceModel(
			await this.getTransaction(options),
		);
		...
		return await targetRepository.aggregate({
			...
			optionsTransformer: (modelOptions) => {
				...
				const throughWhere = {};
				throughWhere[association.foreignKey] = sourceModel.get(
	...
	async create(options?: CreateBelongsToManyOptions): Promise<any> {
		if (Array.isArray(options.values)) {
			return Promise.all(
				options.values.map((record) =>
					this.create({ ...options, values: record }),
				),
			);
		}
	...
	async destroy(
		options?: TargetKey | TargetKey[] | DestroyOptions,
	): Promise<boolean> {
		const transaction = await this.getTransaction(options);
		...
		const throughTableWhere: Array<any> = [
			{
				[throughModel.rawAttributes[association.foreignKey].field]:
					this.sourceKeyValue,
			},
		];
		...
		if (options && options["filterByTk"]) {
			const instances = (<any>this.association).toInstanceArray(
				options["filterByTk"],
			);
			ids = ids
				? lodash.intersection(ids, instancesToIds(instances))
				: instancesToIds(instances);
		}
	...
	async toggle(
		options: TargetKey | { tk?: TargetKey; transaction?: Transaction },
	): Promise<void> {
		const transaction = await this.getTransaction(options);
		const sourceModel = await this.getSourceModel(transaction);
	...
	protected async setTargets(
		...
		const handleKeys: TargetKey[] | PrimaryKeyWithThroughValues[] =
			this.convertTks(options) as any;
