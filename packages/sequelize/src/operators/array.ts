...
export default {
	$match(value, ctx) {
		...
		if (isPg(ctx)) {
			...
			return Sequelize.literal(
				`${name} @> ${queryValue}::JSONB AND ${name} <@ ${queryValue}::JSONB`,
			);
		}
		...
		if (isMySQL(ctx)) {
			const name = queryInterface.quoteIdentifiers(fieldName);
			return Sequelize.literal(
				`JSON_CONTAINS(${name}, ${value}) AND JSON_CONTAINS(${value}, ${name})`,
			);
		}
	...
	$notMatch(value, ctx) {
		...
		if (isPg(ctx)) {
			...
			return Sequelize.literal(
				`not (${name} <@ ${value}::JSONB and ${name} @> ${value}::JSONB)`,
			);
		}
		...
		if (isMySQL(ctx)) {
			const name = queryInterface.quoteIdentifiers(getFieldName(ctx));
			return Sequelize.literal(
				`not (JSON_CONTAINS(${name}, ${value}) AND JSON_CONTAINS(${value}, ${name}))`,
			);
		}
		return {
