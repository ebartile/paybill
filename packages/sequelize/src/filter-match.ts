...
export function filterMatch(model, where) {
	...
	const operatorFunctions = {
		...
		$match: (value, condition) =>
			value.some((item) => filterMatch(item, condition)),
		$notMatch: (value, condition) =>
			!value.some((item) => filterMatch(item, condition)),
		$anyOf: (value, condition) =>
			value.some((item) => condition.includes(item)),
		$noneOf: (value, condition) =>
			!value.some((item) => condition.includes(item)),
		...
		$dateNotBefore: (value, condition) =>
			!moment(value).isBefore(condition, "day"),
		$dateNotAfter: (value, condition) =>
			!moment(value).isAfter(condition, "day"),
		...
		$or: (model, conditions) =>
			Object.values(conditions).some((condition) =>
				filterMatch(model, condition),
			),
