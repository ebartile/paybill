...
export class OptionsParser {
	...
	static appendInheritInspectAttribute(include, collection): any {
		...
		include.push([
			Sequelize.literal(
				`(select relname from pg_class where pg_class.oid = "${collection.name}".tableoid)`,
			),
			"__tableName",
		]);
	...
	filterByTkToWhereOption() {
		...
		const filterTargetKey =
			this.context.targetKey || this.collection.filterTargetKey;
	...
	protected parseSort(filterParams) {
		...
		let defaultSortField: Array<string> | string =
		...
		for (const sortKey of sort) {
			...
			const sortField: Array<any> = sortKey.startsWith("-")
				? sortKey.replace("-", "").split(".")
				: sortKey.split(".");
			...
			if (this.database.isMySQLCompatibleDialect()) {
				...
				if (this.model.fieldRawAttributesMap[fieldName]) {
					orderParams.push([
						Sequelize.fn(
							"ISNULL",
							Sequelize.col(`${this.model.name}.${sortField[0]}`),
						),
					]);
				}
			}
	...
	protected parseFields(filterParams: any) {
		...
		if (this.options?.fields) {
			...
			// Split fields into attributes and appends
	...
	protected parseExcept(except: Except, filterParams: any) {
		...
		const setExcept = (queryParams: any, except: string) => {
			...
			const existIncludeIndex = queryParams["include"].findIndex(
		...
		const setInclude = (
			...
			if (lastLevel) {
				...
				if (
					...
					Array.isArray(existInclude.attributes) &&
					existInclude.attributes.length == 0
				) {
					existInclude.attributes = {
						include: [],
					};
				...
				setInclude(
					model.associations[
						queryParams["include"][existIncludeIndex].association
					].target,
					queryParams["include"][existIncludeIndex],
					nextAppend,
				);
