...
export class ViewFieldInference {
	...
	static async inferFields(options: {
		...
		const columns = await db.sequelize
			.getQueryInterface()
			.describeTable(options.viewName, options.viewSchema);
		...
		for (const [name, column] of Object.entries(columns)) {
			...
			if (usage) {
				...
				const collectionField = (() => {
					...
					const fieldAttribute = Object.values(
				...
				const belongsToAssociationField = (() => {
					...
					const association = Object.values(collection.model.associations).find(
						(association) =>
							...
							association.foreignKey === (field as any).fieldName,
					);
