...
import {
	...
	type BelongsToManyOptions as SequelizeBelongsToManyOptions,
	Utils,
} from "sequelize";
import { Collection } from "../collection";
import { type Reference } from "../features/references-map";
import { checkIdentifier } from "../utils";
import { BelongsToField } from "./belongs-to-field";
import {
	type MultipleRelationFieldOptions,
	RelationField,
} from "./relation-field";
...
export class BelongsToManyField extends RelationField {
	...
	checkAssociationKeys(database) {
		...
		if (!foreignKey) {
			foreignKey = Utils.camelize(
				[Utils.singularize(this.collection.model.name), sourceKey].join("_"),
			);
		}
		...
		if (!otherKey) {
			otherKey = Utils.camelize(
				[Utils.singularize(this.TargetModel.name), targetKey].join("_"),
			);
		}

		const foreignKeyAttribute =
			throughCollection.model.rawAttributes[foreignKey];
	...
	bind() {
		...
		if (!Target.primaryKeyAttribute) {
			throw new Error(
				`Target model ${Target.name} has no primary key attribute`,
			);
		}
		...
		if (database.hasCollection(through)) {
			...
			if (this.collection.options.dumpRules) {
				throughCollectionOptions["dumpRules"] =
					this.collection.options.dumpRules;
			}
		...
		const association = collection.model.belongsToMany(
			Target,
		...
		this.references(association).forEach((reference) =>
	...
	unbind() {
		...
		database.getCollection(this.through);
