...
const defaultIvString = process.env.ENCRYPTION_FIELD_IV || "Xy91$-nPq@7Lm%2Z";
...
export function aseDecryptSync(
	encrypted: string,
	ivString: string = defaultIvString,
) {
	checkValueAndIv("Decrypt", encrypted, ivString);
...
export function aesCheckKey() {
	if (!keyString) {
		throw new EncryptionError(
			"The environment variable `ENCRYPTION_FIELD_KEY` is required, please set it",
		);
	}
	if (typeof keyString !== "string") {
		throw new EncryptionError(
			"The environment variable `ENCRYPTION_FIELD_KEY` must be a string",
		);
	}
	if (keyString.length !== 32) {
		throw new EncryptionError(
			"The environment variable `ENCRYPTION_FIELD_KEY` must be a 32-character string",
		);
	}
}

export function checkValueAndIv(
	...
	if (typeof value !== "string") {
		throw new EncryptionError(
			msg + "The value must be a string, but got " + typeof value,
		);
	}
	...
	if (type === "Decrypt") {
		if (value.length % 2 !== 0) {
			throw new EncryptionError(
				msg +
					`The encrypted value is invalid, not a hex string. The value is "${value}"`,
			);
		}
	}
	...
	if (typeof iv !== "string") {
		throw new EncryptionError(
			msg + "The `iv` must be a string, but got " + typeof iv,
		);
	}
