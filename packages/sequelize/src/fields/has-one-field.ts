...
import {
	...
	type HasOneOptions,
	type HasOneOptions as SequelizeHasOneOptions,
	Utils,
} from "sequelize";
...
export class HasOneField extends RelationField {
	...
	reference(association): Reference {
		...
		return buildReference({
			sourceCollectionName: this.database.modelCollection.get(
				association.target,
			).name,
			sourceField: association.foreignKey,
			targetField: sourceKey,
			targetCollectionName: this.database.modelCollection.get(
				association.source,
			).name,
	...
	unbind() {
		...
		database.removePendingField(this);
		// If no foreign key field is explicitly created in the relationship table, the foreign key is also deleted when the relationship is deleted.
