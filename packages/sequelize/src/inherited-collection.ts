...
export class InheritedCollection extends Collection {
			...
			if (err instanceof ParentCollectionNotFound) {
				const listener = (collection) => {
					if (
						...
						(options.inherits as Array<string>).every((name) =>
							this.db.collections.has(name),
						)
					) {
						this.bindParents();
	...
	parentFields() {
		...
		for (const parent of this.parents) {
			if (parent.isInherited()) {
				for (const [name, field] of (<InheritedCollection>(
					parent
				)).parentFields()) {
	...
	parentAttributes() {
		...
		for (const parent of this.parents) {
			if (parent.isInherited()) {
				Object.assign(
					attributes,
					(<InheritedCollection>parent).parentAttributes(),
				);
			}
