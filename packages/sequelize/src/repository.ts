...
import { isValidFilter, processIncludes } from "./utils";
...
export type CountOptions = Omit<
	SequelizeCountOptions,
	"distinct" | "where" | "include"
> &
	Transactionable & {
		filter?: Filter;
		context?: any;
...
export type FindAndCountOptions = Omit<
...
> {
	...
	constructor(collection: Collection) {
		...
		this.cursorBuilder = new SmartCursorBuilder(
	...
	async count(countOptions?: CountOptions): Promise<number> {
		...
		if (countOptions?.filterByTk) {
			...
			options["where"] = {
				[Op.and]: [
					options["where"] || {},
					optionParser.filterByTkToWhereOption(),
				],
			};
		}
		...
		const queryOptions: any = {
			...options,
			distinct:
				Boolean(this.collection.model.primaryKeyAttribute) &&
				!this.collection.isMultiFilterTargetKey(),
		};

		if (
			Array.isArray(queryOptions.include) &&
			queryOptions.include.length > 0
		) {
			queryOptions.include = processIncludes(
	...
	async getEstimatedRowCount() {
		...
		if (
			_.isFunction(this.collection["isView"]) &&
			this.collection["isView"]()
		) {
			return 0;
		}
      ...
      `,
				...
				return Number(
					results?.[0]?.[
						this.database.inDialect("mysql") ? "TABLE_ROWS" : "table_rows"
					] ?? 0,
				);
			}
      ...
      `,
					{
						replacements: [tableName, this.collection.collectionSchema()],
      ...
      `,
			...
			this.database.logger.error(
				`Failed to get estimated row count for ${this.collection.name}:`,
				error,
			);
			return 0;
		}
	...
	async aggregate(
		options: AggregateOptions & { optionsTransformer?: (options: any) => any },
	): Promise<any> {
	...
	async chunk(
		options: FindOptions & {
			...
			afterFind?: (
				rows: Model[],
				options: FindOptions & { offset: number },
			) => Promise<void>;
		},
	) {
		const {
		...
		while (true) {
			...
			const currentLimit =
				overallLimit !== undefined
					? Math.min(chunkSize, overallLimit - totalProcessed)
					: chunkSize;
			const findOptions = {
				...options,
				limit: currentLimit,
	...
	async find(options: FindOptions = {}) {
		if (
			...
			options?.targetCollection !== this.collection.name
		) {
			return await this.database
				.getCollection(options.targetCollection)
			...
			return await this.update({
				filterByTk: instance.get(
					this.collection.filterTargetKey ||
						this.collection.model.primaryKeyAttribute,
				),
				values,
				transaction,
				context,
	...
	async create(options: CreateOptions) {
		...
		const values = (this.model as typeof Model).callSetters(
		...
		if (options.hooks !== false) {
			...
			await this.database.emitAsync(
				`${this.collection.name}.afterSaveWithAssociations`,
				instance,
				{
					...options,
					transaction,
	...
	async update(options: UpdateOptions & { forceUpdate?: boolean }) {
		...
		if (options.individualHooks === false) {
			...
			const primaryKeyField =
				// @ts-ignore
				Model.primaryKeyField || Model.primaryKeyAttribute;
			// NOTE:
	...
	async destroy(options?: TargetKey | TargetKey[] | DestroyOptions) {
		...
		if (
			...
			if (this.collection.model.primaryKeyAttributes.length > 1) {
				throw new Error(
					`filterByTk is not supported for composite primary key`,
				);
			} else {
				throw new Error(
					`filterByTk is not supported for collection that has no primary key`,
				);
			}
		}
	...
	public buildQueryOptions(options: any) {
		...
		const params = parser.toSequelizeParams({
			parseSort: _.isBoolean(options?.parseSort) ? options.parseSort : true,
		});
