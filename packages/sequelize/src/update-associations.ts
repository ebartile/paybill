...
interface UpdateOptions extends Transactionable {
	...
	// Field blacklist
	blacklist?: string[];
	// By default, relational data will be created and associated. If it is existing data, it will only be associated but not updated.
	// If you need to update associated data, you can specify it through updateAssociationValues
...
function isReverseAssociationPair(a: any, b: any) {
	...
	if (
		(typeSet.has("HasOne") && typeSet.has("BelongsTo")) ||
		(typeSet.has("HasMany") && typeSet.has("BelongsTo"))
	) {
...
export async function updateAssociation(
	...
	if (
		options.associationContext &&
		isReverseAssociationPair(association, options.associationContext)
	) {
		return false;
	}
...
export async function updateSingleAssociation(
	...
	if (
		...
		// @ts-ignore
		!model.get(association.sourceKeyAttribute)
	) {
		throw new Error(
			// @ts-ignore
			`The source key ${association.sourceKeyAttribute} is not set in ${model.constructor.name}`,
		);
	}
	...
	const checkBelongsToForeignKeyValue = () => {
		...
		if (
			...
			!model.get(association.foreignKey)
		) {
			throw new Error(
				// @ts-ignore
...
export async function updateMultipleAssociation(
	...
	for (const item of value) {
		...
		if (isStringOrNumber(item)) {
			...
			const targetKey =
				// @ts-ignore
				(association as any).targetKey || association.options.targetKey || "id";
			...
			if (item[targetKey]) {
				...
				const instance = association.target.build(attributes, {
					isNewRecord: false,
				});
	...
	await model[setAccessor](setItems, {
		transaction,
		context,
		individualHooks: true,
		validate: false,
	...
	if (tmpKey !== pk) {
		const targetKeyFieldOptions = db.getFieldByPath(
			`${association.target.name}.${tmpKey}`,
		)?.options;
	...
	for (const item of objectItems) {
		...
		if (
			pk !== targetKey &&
			!isUndefinedOrNull(item[pk]) &&
			isUndefinedOrNull(item[targetKey])
		) {
	...
	for (const newItem of newItems) {
		const findTargetKey =
			(association as any).targetKey ||
			// @ts-ignore
			association.options.targetKey ||
			targetKey;

		const existIndexInSetItems = setItems.findIndex(
			(setItem) => setItem[findTargetKey] === newItem[findTargetKey],
		);
