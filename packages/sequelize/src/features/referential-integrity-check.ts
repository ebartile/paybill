import { Model, type Transactionable } from "sequelize";
...
export async function referentialIntegrityCheck(
	options: ReferentialIntegrityCheckOptions,
) {
	const { referencedInstance, db, transaction } = options;

	const collection = db.getCollectionByModelName(
		referencedInstance.constructor.name,
	);
	...
	for (const reference of references) {
		const { sourceCollectionName, sourceField, targetField, onDelete } =
			reference;
