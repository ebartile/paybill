...
class ReferencesMap {
	...
	addReference(reference: Reference) {
		...
		if (existReference && existReference.onDelete !== reference.onDelete) {
			...
			if (newPriority > existPriority) {
				...
				newPriority === PRIORITY_MAP["user"]
			) {
				if (
					existReference.onDelete === "SET NULL" &&
	...
	existReference(reference: Reference) {
		...
		const keys = Object.keys(reference).filter(
			(k) => k !== "onDelete" && k !== "priority",
		);

		return references.find((ref) =>
			keys.every((key) => ref[key] === reference[key]),
		);
	}
	...
	removeReference(reference: Reference) {
		...
		this.map.set(
			...
			references.filter(
				(ref) => !keys.every((key) => ref[key] === reference[key]),
			),
		);
	}
}
