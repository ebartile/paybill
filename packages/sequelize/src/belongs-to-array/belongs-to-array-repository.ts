...
export class BelongsToArrayAssociation {
	...
	generateInclude(parentAs?: string) {
		...
		const left = queryInterface.quoteIdentifiers(
			`${asLeft}.${targetField.columnName()}`,
		);
		const right = queryInterface.quoteIdentifiers(
			`${asRight}.${foreignField.columnName()}`,
		);
		return {
...
export class BelongsToArrayRepository extends MultipleRelationRepository {
	...
	constructor(
		...
		this.belongsToArrayAssociation = this
