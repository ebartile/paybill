...
export class SyncRunner {
	...
	async runSync(options) {
		...
		const collectionSyncOptions =
			this.database.collectionFactory.collectionTypes.get(
				// @ts-ignore
		...
		try {
			beforeColumns = await this.queryInterface.describeTable(
				this.tableName,
				options,
			);
		} catch (error) {
		...
		const columns = await this.queryInterface.describeTable(
			this.tableName,
			options,
		);
	...
	async handleUniqueFieldBeforeSync(beforeColumns, options) {
		...
		const newAttributes = Object.keys(this.rawAttributes).filter((key) => {
			return (
				!Object.keys(beforeColumns).includes(this.rawAttributes[key].field) &&
				this.rawAttributes[key].unique
			);
		});
	...
	async handlePrimaryKeyBeforeSync(columns, options) {
		...
		if (
			columnsBePrimaryKey.length == 1 &&
			!columnsWillBePrimaryKey.includes(columnsBePrimaryKey[0])
		) {
			// remove primary key
			if (this.database.inDialect("mariadb", "mysql")) {
				await this.sequelize.query(
					`ALTER TABLE ${this.collection.quotedTableName()} DROP PRIMARY KEY;`,
					options,
				);
			}
		}
	...
	async handlePrimaryKey(columns, options) {
		try {
			...
			if (
				...
				JSON.stringify(columnsBePrimaryKey) !=
					JSON.stringify(columnsWillBePrimaryKey)
			) {
				if (this.database.inDialect("mariadb", "mysql")) {
					await this.sequelize.query(
	...
	async handleDefaultValues(columns, options) {
		...
		for (const columnName in columns) {
			...
			if (columnDefaultValue === null && attributeDefaultValue === undefined)
				continue;
			...
			if (columnDefaultValue != attributeDefaultValue) {
				...
				if (this.database.inDialect("postgres")) {
					...
					const query = this.queryInterface.queryGenerator.attributesToSQL(
						{
							[columnName]:
								// @ts-ignore
								this.queryInterface.normalizeAttribute(changeAttribute),
						},
						{
					...
					// @ts-ignore
					const sql = this.queryInterface.queryGenerator.changeColumnQuery(
						this.tableName,
						query,
					);
	...
	async handleUniqueIndex(options) {
		...
		const existsIndexes: any = await this.queryInterface.showIndex(
			this.collection.getTableNameWithSchema(),
			options,
		);
		...
		for (const existUniqueIndex of existsUniqueIndexes) {
			...
			if (
				!currentAttribute ||
				(!currentAttribute.unique && !currentAttribute.primaryKey)
			) {
				if (this.database.inDialect("postgres")) {
					// @ts-ignore
					const constraints = await this.queryInterface.showConstraint(
						this.tableName,
						existUniqueIndex.name,
						options,
					);
					if (
						constraints.some((c) => c.constraintName === existUniqueIndex.name)
					) {
		...
		for (const uniqueAttribute of uniqueAttributes) {
			...
			const indexExists = existsUniqueIndexes.find((index) => {
				return (
					...
					index.fields[0].attribute == this.rawAttributes[uniqueAttribute].field
				);
			});
			...
			if (!indexExists) {
				await this.queryInterface.addIndex(
					this.tableName,
					[this.rawAttributes[uniqueAttribute].field],
					{
						unique: true,
						transaction: options?.transaction,
