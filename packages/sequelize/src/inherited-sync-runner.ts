...
export class InheritedSyncRunner {
	static async syncInheritModel(model: any, options: any) {
		...
		if (
			...
			if (childAttributes.id && childAttributes.id.autoIncrement) {
             ...
             WHERE table_name = '${parent.model.tableName}'
               ...
               and "column_name" = 'id';`,
					...
					if (!columnDefault) {
						throw new Error(
							`Can't find sequence name of parent collection ${parent.options.name}`,
						);
					}
			...
			if (maxSequenceName) {
				const parentsDeep = Array.from(
					db.inheritanceMap.getParents(inheritedCollection.name),
				).map((parent) => db.getCollection(parent).getTableNameWithSchema());
	...
	static async createTable(tableName, attributes, options, model, parents) {
		...
		attributes = lodash.mapValues(attributes, (attribute) =>
		...
		sql =
			`${queryGenerator.createTableQuery(tableName, attributes, options)}`.replace(
				";",
				` INHERITS (${parents
					.map((t) => {
