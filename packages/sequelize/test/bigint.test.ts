...
describe("collection", () => {
	...
	it("should using bigint for id field", async () => {
		...
		const tableInfo = await db.sequelize
			.getQueryInterface()
			.describeTable(collection.model.tableName);
		...
		const profileTableInfo = await db.sequelize
			.getQueryInterface()
			.describeTable(profile.model.tableName);

		if (db.inDialect("mariadb")) {
			expect(
				profileTableInfo[profile.model.rawAttributes["userId"].field].type,
			).toBe("BIGINT(20)");
		} else {
			expect(
				profileTableInfo[profile.model.rawAttributes["userId"].field].type,
