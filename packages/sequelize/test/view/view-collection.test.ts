import { Database, ViewCollection, mockDatabase } from "@paybilldev/sequelize";
import { uid } from "@paybilldev/sequelize";
			...
			fields: [
				{
					name: "name",
					type: "string",
					interface: { type: "string", title: "Name" },
				},
				{
					name: "group",
					type: "belongsTo",
			...
			fields: [
				{
					name: "name",
					type: "string",
					interface: { type: "string", title: "Group name" },
				},
			],
		});
...
DECLARE
  new_group_id BIGINT;
BEGIN
  -- Insert a new groups row and get the ID of the newly inserted row
  INSERT INTO ${GroupCollection.quotedTableName()} (name) VALUES (NEW.group_name) RETURNING id INTO new_group_id;

  -- Insert a new users row, using the ID of the newly inserted groups row as group_id
...
describe("create view", () => {
    ...
    `;
		...
		expect(viewNameField.options.patterns).toEqual(
			UserCollection.getField("name").options.patterns,
		);
	});
