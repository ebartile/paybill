import { mockDatabase } from "@paybilldev/sequelize";
import { Collection } from "@paybilldev/sequelize";
...
describe("estimated row count test", () => {
	...
	test("getEstimatedRowCount with different table naming conventions", async () => {
		...
		const snakeCaseCount =
			await SnakeCaseCollection.repository.getEstimatedRowCount();
		expect(snakeCaseCount).toBe(2);

		await analyzeTable(db, CamelCaseCollection);
		const camelCaseCount =
			await CamelCaseCollection.repository.getEstimatedRowCount();
...
async function analyzeTable(db, collection) {
	if (db.isMySQLCompatibleDialect()) {
		await db.sequelize.query(
			`ANALYZE TABLE ${collection.getTableNameWithSchema()}`,
		);
