...
import { InheritedCollection } from "@paybilldev/sequelize";
...
describe("collection inherits", () => {
	...
	it("should append collection name in eager load", async () => {
		...
		expect(assoc.get("__schemaName")).toEqual(
			childCollection.collectionSchema(),
		);
	...
	it("should list data filtered by child type", async () => {
		...
		expect(
			records.every((r) => r.get("__collection") === child1Collection.name),
		).toBe(true);
		...
		expect(
			records2.every((r) => r.get("__collection") !== child1Collection.name),
		).toBe(true);
		...
		expect(
			recordsWithFilter.every(
				(r) => r.get("__collection") == child1Collection.name,
			),
		).toBe(true);
	...
	it("should update inherit field when parent field update", async () => {
		...
		expect(db.getCollection("students").getField("name").get("title")).toBe(
			"parent-name",
		);
		...
		expect(db.getCollection("person").getField("name").get("title")).toBe(
			"new-name",
		);
		expect(db.getCollection("students").getField("name").get("title")).toBe(
			"new-name",
	...
	it("should create inherited table", async () => {
		...
		const studentTableInfo = await db.sequelize
			.getQueryInterface()
			.describeTable(student.model.tableName);
