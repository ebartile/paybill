...
describe("belongs to many with targetCollection", () => {
	...
	it("should update child collection", async () => {
		...
		const repository = Org.repository
			.relation<BelongsToManyRepository>("users")
			.of(org1.get("id"));
...
describe("belongs to many with collection that has no id key", () => {
	...
	it("should set relation", async () => {
		...
		const a1bsRepository = await A.repository
			.relation<BelongsToManyRepository>("bs")
			.of("a1");
...
describe("belongs to many with target key", function () {
	...
	test("destroy by target key", async () => {
		...
		const PostTagRepository = new BelongsToManyRepository(
			Post,
			"tags",
			p1.get("title") as string,
		);
...
describe("belongs to many", () => {
	...
	test("update through table attribute", async () => {
		...
		// Set the tagged_at associated with p1 and t1
		await PostTagRepository.update({
			filter: {
				name: "t1",
			},
	...
	test("update association values", async () => {
		...
		const tagPostsRepository = new BelongsToManyRepository(
			Tag,
			"posts",
			tag.id,
		);
