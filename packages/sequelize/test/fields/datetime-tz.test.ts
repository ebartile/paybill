import { Database, Repository, mockDatabase } from "@paybilldev/sequelize";
...
describe("timezone", () => {
	...
	it("should save with timezone value", async () => {
		...
		const instance = await repository.create({
			values: { date1: "2023-03-23T12:00:00.000Z" },
		});
			...
			fields: [
				{
					name: "date1",
					type: "datetimeTz",
					defaultValue: "2023-03-23T18:00:00.000Z",
				},
			],
		});
	...
	describe("timezone", () => {
		test("custom", async () => {
			...
			const repository = db.getRepository("tests");
			const instance = await repository.create({
				values: { date1: "2023-03-24 00:00:00" },
			});
			const date1 = instance.get("date1");
			expect(date1.toISOString()).toEqual("2023-03-23T18:00:00.000Z");
		});
		...
		test("server", async () => {
			...
			const repository = db.getRepository("tests");
			const instance = await repository.create({
				values: { date1: "2023-03-24 08:00:00" },
			});
			const date1 = instance.get("date1");
			expect(date1.toISOString()).toEqual("2023-03-24T00:00:00.000Z");
		});
...
describe("date-field", () => {
	...
	it("should set default value if collection is middle table in belongs to many association", async () => {
		...
		const sourceInstance = await sourceTable.repository.create({
			values: { name: "source" },
		});
		const targetInstance = await targetTable.repository.create({
			values: { name: "target" },
		});
