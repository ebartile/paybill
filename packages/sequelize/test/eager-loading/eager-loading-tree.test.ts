...
import { EagerLoadingTree } from "@paybilldev/sequelize";
...
describe("Eager loading tree", () => {
	...
	it("should sort has many default by primary key", async () => {
		...
		await Source.repository.create({
			values: {
				name: "s1",
				targets: [{ id: target2.get("id") }, { id: target1.get("id") }],
			},
		});
		...
		expect(source.get("targets").map((item: any) => item.get("id"))).toEqual([
			1, 2,
		]);
	});
	...
	it("should build eager loading tree", async () => {
		...
		expect(
			eagerLoadingTree.root.children[0].children[0].children[0].model,
		).toBe(TagCategory.model);

		await eagerLoadingTree.load();

		expect(eagerLoadingTree.root.instances).toHaveLength(2);
		const u1 = eagerLoadingTree.root.instances.find(
			(item) => item.get("name") === "u1",
		);
		...
		expect(
			u1.get("posts")[0].get("tags")[0].get("tagCategory").get("name"),
		).toBe("c1");
	});
});
